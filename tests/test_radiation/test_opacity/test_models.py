"""Tests for :mod:`triceratops.radiation.opacity.models.core`.

Organisation
------------
TestNormalisationConstants      — KAPPA_FF_0, KAPPA_BF_0, KAPPA_KR_0 consistency
TestConstantOpacity             — fixed κ, zero derivatives, .kappa property
TestElectronScatteringOpacity   — constant Thomson κ, IS_C_BACKED, zero derivatives
TestKramersFFOpacity            — κ ∝ ρ T^{-3.5}, unit derivatives, default kappa0
...
TestKramersFFESOpacity          — κ = κ_es + κ_ff·ρ·T^{-3.5}, mixed derivatives
TestKramersBFESOpacity          — κ = κ_es + κ_bf·ρ·T^{-3.5}
TestKramersESOpacity            — κ = κ_es + (κ_ff+κ_bf)·ρ·T^{-3.5}
TestKramersPurePowerLawScaling  — parametrised ρ- and T-scaling for all pure Kramers laws
...
class TestConstantOpacity:
    """Tests for :class:`~triceratops.radiation.opacity.models.core.ConstantOpacity`.

    Verifies that the opacity value is fixed and that both partial
    derivatives are identically zero regardless of (ρ, T).
    """

    def test_is_grey_opacity_law(self):
        """ConstantOpacity inherits from GreyOpacityLaw."""
        assert isinstance(ConstantOpacity(0.5), GreyOpacityLaw)
    ...
    def test_kappa_property_matches_constructor(self):
        ...
        assert law.kappa == pytest.approx(0.5, rel=1e-12)

    def test_opacity_returns_correct_value(self):
        """opacity() returns the constructor κ in cm² g⁻¹."""
        law = ConstantOpacity(0.5)
        result = law.opacity(_RHO, _T)
        assert result.value == pytest.approx(0.5, rel=1e-12)

    def test_opacity_independent_of_rho(self):
        """opacity() is constant: changing ρ by six orders of magnitude has no effect."""
        law = ConstantOpacity(0.5)
    ...
    def test_is_not_c_backed(self):
        """ConstantOpacity is a pure-Python implementation (IS_C_BACKED = False)."""
        assert ConstantOpacity.IS_C_BACKED is False
...
class TestElectronScatteringOpacity:
    """Tests for :class:`~triceratops.radiation.opacity.models.core.ElectronScatteringOpacity`.

    Thomson scattering produces a grey (constant) opacity; the value does
    not depend on ρ or T, and both partial derivatives vanish.
    """
    ...
    def test_default_kappa_es(self):
        """The default electron-scattering opacity is 0.34 cm² g⁻¹."""
        law = ElectronScatteringOpacity()
    ...
    def test_opacity_matches_kappa_es(self):
        """opacity() returns the stored kappa_es value in cm² g⁻¹."""
        kap_es = 0.34
    ...
    def test_opacity_independent_of_rho(self):
        """Thomson opacity is constant — ρ has no effect."""
        law = ElectronScatteringOpacity()
        kap1 = law.opacity(1.0e-10 * u.g / u.cm**3, _T)
    ...
    def test_dlogkappa_dlogrho_zero(self):
        """d ln κ / d ln ρ = 0 for electron scattering."""
        law = ElectronScatteringOpacity()
        assert law.dlogkappa_dlogrho(_RHO, _T) == pytest.approx(0.0, abs=1e-12)

    def test_dlogkappa_dlogT_zero(self):
        """d ln κ / d ln T = 0 for electron scattering."""
        law = ElectronScatteringOpacity()
...
class TestKramersFFOpacity:
    """Tests for :class:`~triceratops.radiation.opacity.models.core.KramersFFOpacity`.

    The free-free Kramers law obeys κ = κ₀ ρ T^{-3.5}, giving
    d ln κ / d ln ρ = 1 and d ln κ / d ln T = −3.5 everywhere.
    """
    ...
    def test_opacity_formula(self):
        """opacity() matches κ₀ ρ T^{-3.5} at the reference point."""
        law = KramersFFOpacity()
...
class TestKramersBFOpacity:
    """Tests for :class:`~triceratops.radiation.opacity.models.core.KramersBFOpacity`.

    Bound-free Kramers: κ = κ_bf,0 ρ T^{-3.5}.  Same power-law structure
    ...
    def test_opacity_formula(self):
        """opacity() matches κ_bf,0 ρ T^{-3.5} at the reference point."""
        law = KramersBFOpacity()
...
class TestKramersOpacity:
    """Tests for :class:`~triceratops.radiation.opacity.models.core.KramersOpacity`.

    Combined (FF + BF) Kramers law: κ = (κ_ff,0 + κ_bf,0) ρ T^{-3.5}.
...
def _kramers_es_dlogT(kappa_kr, rho_cgs, T_cgs, kappa_es=0.34):
    """d ln κ / d ln T = −3.5 · κ_kr·ρ·T^{-3.5} / κ_total."""
    return -3.5 * _kramers_es_dlogrho(kappa_kr, rho_cgs, T_cgs, kappa_es)


class TestKramersFFESOpacity:
    """Tests for :class:`~triceratops.radiation.opacity.models.core.KramersFFESOpacity`.

    κ = κ_es + κ_ff,0 · ρ · T^{-3.5}.  Both attributes (kappa0, kappa_es)
    ...
    def test_default_kappa_es(self):
        """Default kappa_es equals 0.34 cm² g⁻¹."""
        law = KramersFFESOpacity()
        assert law.kappa_es == pytest.approx(0.34, rel=1e-12)

    def test_opacity_formula(self):
        """opacity() matches κ_es + κ_ff,0 · ρ · T^{-3.5} at the reference point."""
        law = KramersFFESOpacity()
    ...
    def test_dlogkappa_dlogrho_formula(self):
        """d ln κ / d ln ρ matches the analytical formula."""
        law = KramersFFESOpacity()
    ...
    def test_dlogkappa_dlogT_formula(self):
        """d ln κ / d ln T matches the analytical formula."""
        law = KramersFFESOpacity()
    ...
    def test_dlogkappa_dlogrho_between_zero_and_one(self):
        """d ln κ / d ln ρ lies in (0, 1) at the reference point."""
        law = KramersFFESOpacity()
        val = law.dlogkappa_dlogrho(_RHO, _T)
        assert 0.0 < val < 1.0

    def test_dlogkappa_dlogT_between_minus_3_5_and_zero(self):
        """d ln κ / d ln T lies in (−3.5, 0) at the reference point."""
        law = KramersFFESOpacity()
...
class TestKramersBFESOpacity:
    """Tests for :class:`~triceratops.radiation.opacity.models.core.KramersBFESOpacity`.

    κ = κ_es + κ_bf,0 · ρ · T^{-3.5}.
    """
    ...
    def test_opacity_formula(self):
        """opacity() matches κ_es + κ_bf,0 · ρ · T^{-3.5}."""
        law = KramersBFESOpacity()
...
class TestKramersESOpacity:
    """Tests for :class:`~triceratops.radiation.opacity.models.core.KramersESOpacity`.

    κ = κ_es + (κ_ff,0 + κ_bf,0) · ρ · T^{-3.5}.  This is the most
    ...
    def test_opacity_formula(self):
        """opacity() matches κ_es + KAPPA_KR_0 · ρ · T^{-3.5}."""
        law = KramersESOpacity()
...
class TestKramersPurePowerLawScaling:
    ...
    For κ = κ₀ ρ T^{-3.5} we expect:
    - Doubling ρ doubles κ (linear in density).
    - Reducing T by factor 2 increases κ by 2^{3.5} (negative temperature index).
    """

    def test_opacity_doubles_with_rho(self, LawClass, kappa0_attr, kappa0_val):
        """Doubling ρ doubles the opacity (linear density dependence)."""
        law = LawClass()
    ...
    def test_dlogkappa_dlogrho_unity_at_multiple_points(self, LawClass, kappa0_attr, kappa0_val):
        """d ln κ / d ln ρ = 1 at several (ρ, T) pairs."""
        law = LawClass()
    ...
    def test_dlogkappa_dlogT_minus_3_5_at_multiple_points(self, LawClass, kappa0_attr, kappa0_val):
        """d ln κ / d ln T = −3.5 at several (ρ, T) pairs."""
        law = LawClass()
        for rho in [1e-8, 1e-5, 1e-2]:
            for T in [1e5, 1e7, 1e9]:
...
class TestKramersESDerivativeLimits:
    ...
    In the ES-dominated regime (low ρ or high T), derivatives approach zero.
    In the Kramers-dominated regime (high ρ or low T), derivatives approach
    the pure Kramers values (d/dρ → 1, d/dT → −3.5).
    """

    def test_es_dominated_dlogrho_near_zero(self, LawClass, kappa_kr):
        """d ln κ / d ln ρ → 0 when electron scattering dominates (very low ρ)."""
        law = LawClass()
        # At very low ρ, Kramers term ≪ κ_es → derivative → 0
    ...
    def test_es_dominated_dlogT_near_zero(self, LawClass, kappa_kr):
        """d ln κ / d ln T → 0 when electron scattering dominates (very low ρ)."""
        law = LawClass()
        rho_low = 1.0e-15 * u.g / u.cm**3
        T_high = 1.0e10 * u.K
    ...
    def test_kramers_dominated_dlogrho_near_unity(self, LawClass, kappa_kr):
        """d ln κ / d ln ρ → 1 when Kramers term dominates (very high ρ)."""
        law = LawClass()
        # At very high ρ, Kramers term ≫ κ_es → derivative → 1
    ...
    def test_kramers_dominated_dlogT_near_minus_3_5(self, LawClass, kappa_kr):
        """d ln κ / d ln T → −3.5 when Kramers term dominates (very high ρ)."""
        law = LawClass()
        rho_high = 1.0e5 * u.g / u.cm**3
        T_low = 1.0e4 * u.K
    ...
    def test_dlogkappa_dlogrho_dlogT_consistent(self, LawClass, kappa_kr):
        """d ln κ / d ln T = −3.5 · d ln κ / d ln ρ at all points (both derived from same Kramers fraction)."""
        law = LawClass()
        for rho in [1e-8, 1e-5, 1e-2]:
            for T in [1e5, 1e7, 1e9]:
