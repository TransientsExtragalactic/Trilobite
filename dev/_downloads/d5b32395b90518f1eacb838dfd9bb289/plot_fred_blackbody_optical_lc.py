...
- :math:`\phi_\mathrm{FRED}` is the dimensionless FRED temporal profile
  (peak value = 1, onset at :math:`t_0`, rise timescale :math:`\tau_r`,
  decay timescale :math:`\tau_d`),
- :math:`\mathbf{W}` is the filter weight matrix from
...
- :ref:`optical_light_curve` — data container for single-band optical time series
- Gallery: :ref:`sphx_glr_galleries_inference_plot_optical_lc_inference.py` — fitting this model to data
"""
...
# precomputes the weight matrix :math:`\mathbf{W}` once so that all subsequent
...
# internally.
#
# See :ref:`sphx_glr_galleries_photometry_plot_filter_bundle.py` for a deeper
...
# dimensionless FRED temporal normalization :math:`\phi(t)` that modulates the
# SED amplitude:
#
# .. math::
#
...
# %%
# The peak time of the FRED profile is determined implicitly by :math:`\tau_r`
# and :math:`\tau_d`. A fast rise (small :math:`\tau_r`) combined with a slow
# decay (large :math:`\tau_d`) gives an asymmetric profile characteristic of
...
# %%
# Because the blackbody SED peaks near the ultraviolet at :math:`T_\mathrm{eff}
...
# **colour evolution**. Even though :math:`\phi(t)` is band-independent, the
...
#
# Here we compute the :math:`g - r` colour as a function of time and compare
...
# %%
# Because the temperature is fixed in this model variant, the :math:`g - r`
# colour is constant — it depends only on :math:`T_\mathrm{eff}` (and the filter
...
# additional shape exponents :math:`\nu_r` and :math:`\nu_d`.

# %%
# Parameter Sensitivity
# ----------------------
#
# Below we sweep :math:`T_\mathrm{eff}` to show how the blackbody temperature
# controls the :math:`u - z` colour span, illustrating the model's ability to
...
# %%
# A hotter blackbody (larger :math:`T_\mathrm{eff}`) is bluer: its *u*-band
# flux is enhanced relative to the red bands.  At :math:`T_\mathrm{eff} \sim
...
# allows Bayesian inference to constrain :math:`T_\mathrm{eff}` from
# multi-band photometry even when individual bands are noisy.
#
# To see how this model is fitted to data with MCMC, see
# :ref:`sphx_glr_galleries_inference_plot_optical_lc_inference.py`.
