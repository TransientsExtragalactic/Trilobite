"""Cython declarations for C_OPALTableOpacity."""

from ..opacity_base cimport C_GreyOpacityBase

cdef class C_OPALTableOpacity(C_GreyOpacityBase):
    # log10(T [K]) grid, shape (n1,)
    cdef double[::1]    _g1
    # log10(R) or log10(rho) grid, shape (n2,)
    cdef double[::1]    _g2
    # log10(kappa [cm**2/g]) table, shape (n1, n2); NaN = invalid
    cdef double[:, ::1] _lk
    cdef int _n1
    cdef int _n2
    # coordinate system: 0 = T_R, 1 = T_rho
    cdef int _coord
