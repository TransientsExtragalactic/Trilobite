...
:class:`~triceratops.dynamics.accretion.one_zone.closure.OneZoneClosure`
...
:mod:`triceratops.dynamics.accretion.one_zone.closure` (``.pxd``); import
...
.. code-block:: python
    ...
    class TestMyClosure(BaseTestOneZoneDisk):
        ...
        # Override this if your closure packs more than the 4 base parameters.
        def test_pack_cython_parameters_shape(self):
            disk = self._disk()
            params = disk._pack_cython_parameters(
                disk.process_runtime_parameters(self._runtime_parameters())
            )
            assert params.shape == (5,)  # adjust to match your parameter count
