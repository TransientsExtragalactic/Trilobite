r"""
Equation of state (EOS) utilities for Trilobite models.
...
:mod:`~trilobite.dynamics.accretion.one_zone.physics._eos`.  The
...
See Also
--------
:mod:`trilobite.physics_utils.composition` : Mean molecular weight helpers used to set ``mu``.
:mod:`trilobite.physics_utils.constants` : CGS constants consumed internally.
:mod:`~trilobite.dynamics.accretion.one_zone.physics._eos` : Cython EOS
...
def _log_ideal_gas_disk_sound_speed(
    ...
    :mod:`~trilobite.dynamics.accretion.one_zone.physics._eos`.
...
def ideal_gas_sound_speed(T: u.Quantity, mu: float) -> u.Quantity:
    ...
    .. code-block:: python
        ...
        from trilobite.physics_utils.eos import (
...
class EquationOfState(ABC):
    ...
    **params : dict
    ...
    Notes
    -----
    The two-level API mirrors the rest of the Trilobite codebase:
