r"""
TOPS Rosseland Mean Opacity: :math:`\kappa(T,\rho)` for Solar Composition
==========================================================================
...
:math:`(\log_{10}T,\,\log_{10}R)` grid where
:math:`R \equiv \rho\,T_6^{-3}` — the TOPS table is stored on a direct
:math:`(\log_{10}T,\,\log_{10}\rho)` grid, so a fixed density corresponds
...
.. note::
   ...
   The bundled TOPS table covers
   :math:`5\times10^{-4}\;\mathrm{keV} \lesssim T \lesssim 100\;\mathrm{keV}`
   (:math:`\approx 5.8\times10^3\;\mathrm{K}` to :math:`1.16\times10^9\;\mathrm{K}`)
   and :math:`10^{-7} \le \rho\,[\mathrm{g\,cm^{-3}}] \le 10^2`.
...
# -----------------------------
#
# We sweep temperature from :math:`10^3\;\mathrm{K}` to :math:`2\times10^9\;\mathrm{K}` —
...
# and appear as gaps.
#
# Because the TOPS table is stored on a direct :math:`(\log_{10}T,\,\log_{10}\rho)`
# grid (unlike OPAL's :math:`(\log_{10}T,\,\log_{10}R)` grid), each density
# curve traces a *horizontal slice* through the table; there is no
# :math:`R`-direction out-of-bounds effect as density is fixed.
...
# TOPS uses a direct :math:`(\log_{10}T,\,\log_{10}\rho)` grid, so fixed-density
# curves stay within the table for the full temperature range without the
# :math:`R`-direction out-of-bounds gaps seen in OPAL.  TOPS also covers a
# wider temperature range (up to :math:`\sim 10^9\;\mathrm{K}`) and a denser
# density grid (100 points over :math:`10^{-7}`–:math:`10^2\;\mathrm{g\,cm^{-3}}`).
