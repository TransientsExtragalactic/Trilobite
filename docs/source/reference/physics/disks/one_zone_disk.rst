...
.. code-block:: python
    ...
    from triceratops.dynamics.accretion.one_zone import GasPressureElectronScatteringDisk
...
.. tab-set::
    ...
    .. tab-item:: GasPressureElectronScatteringDisk
    ...
    .. tab-item:: FullPressureElectronScatteringDisk
...
The following fields are available in ``result.data`` for both
:class:`~triceratops.dynamics.accretion.one_zone.core.GasPressureElectronScatteringDisk`
and
:class:`~triceratops.dynamics.accretion.one_zone.core.FullPressureElectronScatteringDisk`:
...
.. code-block:: python
    ...
    # {"target": "...core:GasPressureElectronScatteringDisk", "mu": 0.62}

    disk2 = GasPressureElectronScatteringDisk.from_spec_dict(spec)
...
Developer Guide: Adding a New Closure
--------------------------------------

Adding a new one-zone disk closure requires three steps: writing the Cython
extension, declaring the Python model class, and registering the tests.

Step 1 — Write the Cython closure
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Each closure is a subclass of
:class:`~triceratops.dynamics.accretion.one_zone._integrator.OneZoneClosure`
defined in a ``.pyx`` file.  The subclass must install three C function pointers
in its ``__cinit__``:

- ``_closure_fn`` — evaluates the energy balance, solves for :math:`T_c`, and
  fills a ``ClosureResult`` struct.
- ``_derivative_fn`` — evaluates :math:`(\dot{M}_D, \dot{J}_D)` and computes the
  adaptive timestep.
- ``_writer_fn`` — writes all output quantities to the result array at the current
  column index.

The function signatures are declared in
:mod:`triceratops.dynamics.accretion.one_zone._integrator` (``.pxd``); import
them with ``cimport``.  The ``ClosureResult`` struct carries the standard set of
thermodynamic outputs:

.. code-block:: c

    cdef struct ClosureResult:
        double dM_dt       # g s^-1  (negative: drain)
        double dJ_dt       # g cm^2 s^-2
        double t_visc      # s   (warm-start seed for next step)
...
The writer function must also fill any extra rows specific to your closure.
Set ``n_result_fields`` in ``__cinit__`` to the total number of rows you intend
to write.

Step 2 — Write the Python model class
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Subclass
:class:`~triceratops.dynamics.accretion.one_zone.base.OneZoneAccretionDiskBase`
and declare the four required class-level dictionaries:

.. code-block:: python

    class MyClosure(OneZoneAccretionDiskBase):

        # Geometry constants (must match the compiled Cython layer)
        _A: float = 1.62
        _B: float = 1.33
        _F: float = 1.6
        ...
        # Parameters passed to each solve() call
        RUNTIME_PARAMETERS = {
            "M_BH":  {"description": "BH mass",            "base_units": "g",  "default": None, "log_transform": True},
            "R_in":  {"description": "Inner radius",        "base_units": "cm", "default": None, "log_transform": True},
            "alpha": {"description": "Shakura-Sunyaev alpha","base_units": None,"default": None, "log_transform": False},
        }

        # Initial conditions consumed by solve()
        INITIAL_CONDITIONS = {
            "M_D_0": {"description": "Initial disk mass",     "base_units": "g",         "default": None, "log_transform": True},
            "J_D_0": {"description": "Initial angular momentum","base_units": "g*cm**2/s","default": None, "log_transform": True},
        }

        # All output fields
        RESULT_FIELDS = {
            "R_D":   {"description": "Disk outer radius",  "units": "cm"},
            "T_c":   {"description": "Midplane temperature","units": "K"},
            # ... add your own fields
        }

        # Maps RESULT_FIELDS keys → row index in the Cython result array
        # (None = field not directly in result_array)
        CYTHON_FIELD_MAP = {
            "R_D": 4,
            "T_c": 8,
            # ...
        ...
        def _build_cython_closure(self):
            from ._my_closure import MyCythonClosure
            return MyCythonClosure()

        def _pack_cython_parameters(self, run_params):
            return np.array([
                np.exp(run_params["log_M_BH"]),
                np.exp(run_params["log_R_in"]),
                run_params["alpha"],
                self._context_parameters["mu"],
                # ... any additional parameters your closure needs
            ], dtype=np.float64)

        def to_spec_dict(self):
            return {"target": "...:MyClosure", "mu": self._context_parameters["mu"]}
...
.. important::
    ...
    validates the four declaration dicts **at class-definition time**.  If a
    required key is missing or has the wrong type, you will see a descriptive
    :exc:`TypeError` immediately — not buried inside a solver traceback.

Step 3 — Add tests
^^^^^^^^^^^^^^^^^^^^

The test module
``tests/test_dynamics/test_one_zone_disk_base.py`` provides
``BaseTestOneZoneDisk``, a shared pytest base class that runs the full suite of
physical consistency and round-trip checks for any concrete disk model.  To add
tests for your new closure:

.. code-block:: python

    # tests/test_dynamics/test_my_closure.py
    from test_one_zone_disk_base import BaseTestOneZoneDisk
    from triceratops.dynamics.accretion.one_zone import MyClosure

    class TestMyClosure(BaseTestOneZoneDisk):
        MODEL = MyClosure
        DISK_KWARGS = {"mu": 0.62}
        ...
        # Add any closure-specific tests below ...
        def test_my_specific_physics(self):
            ...
