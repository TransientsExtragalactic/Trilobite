"""
Photometry utilities for interacting with and processing photometric data in Triceratops.
...
class FilterBundle:
    ...
    def plot(
        ...
        **kwargs
        ...
        axes : matplotlib.axes.Axes
        """
        from triceratops.utils.plot_utils import get_default_cmap, resolve_fig_axes, set_plot_style
...
def load_filter_from_speclite(name: str, weighting: str = "photon") -> PhotometryFilter:
    ...
    try:
        ...
        raise ImportError(
            "speclite is required to load filters by name. Install it with: pip install triceratops[optical]"
        ) from None
...
def list_speclite_filters(group: Optional[str] = None) -> list:
    ...
    try:
        ...
        raise ImportError("speclite is required. Install it with: pip install triceratops[optical]") from None
